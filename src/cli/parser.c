...
// Converting a string to a double using strtod()
// returns 1 on full valid conversion, 0 if any trailing/invalid chars remain
...
// Setting result_t in one place to standardize the way every dispatcher returns results
static result_t set_result(result_status status, double value, const char *msg) {
	result_t r;
	r.status = status;
	r.result_val = value;
	snprintf(r.error_message, sizeof(r.error_message), "%s", msg); // snprintf, not sprintf: caps msg at buffer size, no overflow risk
	return r;
}
...
// Handling all single-operand commands (sqroot, factorial): parse -> call -> classify result
static result_t dispatch_unary(int argc, char *argv[], unary_op fn, const char *usage) {
	if (argc != 3) { // command + 1 operand expected
...
// Handling all two-operand commands (add, subtract, power, log, etc.): parse -> call -> classify result
static result_t dispatch_binary(int argc, char *argv[], binary_op fn, const char *usage) {
	if (argc != 4) { // command + 2 operands expected
		return set_result(MATHINATOR_INVALID, NAN, usage);
	}
	double a, b;
...
// Normalising an angle in degrees to [0, 360) then converting to radians
static double deg_to_rad(double deg) {
	deg = fmod(deg, 360.0);
	if (deg < 0) // fmod can return negative for negative input, wrapping into positive range
		deg += 360.0;
	return deg * DEG_TO_RAD;
}

// Handling all trigonometry commands: adds --deg/--rad flag parsing on top of the unary pattern
static result_t dispatch_trigo(int argc, char *argv[], unary_op fn, const char *usage) {
	if ((argc != 3) && (argc != 4)) { // command + angle, optionally + flag
		return set_result(MATHINATOR_INVALID, NAN, usage);
	}
	if ((argc == 4) && (strcmp(argv[3], "--rad") != 0 && strcmp(argv[3], "--deg") != 0)) {
		// 4th arg present but isn't a recognised flag
		return set_result(MATHINATOR_INVALID, NAN, usage);
	}
	double angle;
	...
	if ((argc == 3) || ((argc == 4) && (strcmp(argv[3], "--deg") == 0))) {
		// no flag defaults to degrees; explicit --deg also converts. --rad falls through untouched.
...
// Handling all statistics commands (mean, median, mode, variance, stddev): variable-length dataset instead of fixed operand count
static result_t dispatch_stats(int argc, char *argv[], stats_op fn, const char *usage) {
	if (argc < 3) { // need at least command + 1 value
		return set_result(MATHINATOR_INVALID, NAN, usage);
	}
	if (argc > MAX_CLI_ARGS + 2) { // +2 accounts for argv[0] (program name) and argv[1] (command name)
		return set_result(MATHINATOR_INVALID, NAN, "Too many operands. Maximum 20 elements allowed");
	}
	int sample_size = argc - 2; // operand count, excluding program name and command name
...
// High-level command router: matches argv[1] against every known command and forwards to the right dispatcher
result_t parse_and_dispatch(int argc, char *argv[]) {
	set_result(MATHINATOR_INVALID, NAN, "Unknown error");
	if (argc < 2) { // no command given at all
	...
	if (strcmp(argv[1], "add") == 0) {
		...
		return dispatch_stats(argc, argv, stats_mean, "Usage: mathinator mean <n1> <n2> ... <n20>");
	} else if (strcmp(argv[1], "median") == 0) {
		return dispatch_stats(argc, argv, stats_median, "Usage: mathinator median <n1> <n2> ... <n20>");
	} else if (strcmp(argv[1], "mode") == 0) {
		return dispatch_stats(argc, argv, stats_mode, "Usage: mathinator mode <n1> <n2> ... <n20>");
	} else if (strcmp(argv[1], "variance") == 0) {
		return dispatch_stats(argc, argv, stats_variance, "Usage: mathinator variance <n1> <n2> ... <n20>");
	} else if (strcmp(argv[1], "stddev") == 0) {
		return dispatch_stats(argc, argv, stats_stddev, "Usage: mathinator stddev <n1> <n2> ... <n20>");
	} else {
		// unrecognised command name: build a help message listing what IS available
		char msg[256];
