...
int main(int argc, char* argv[]) {
	result_t answer = parse_and_dispatch(argc, argv);
	if (answer.status == MATHINATOR_INVALID) {
		fprintf(stderr, "%s\n", answer.error_message);
		return MATHINATOR_EXIT_INVALID;
	}
	if (answer.status == MATHINATOR_UNDEF){
		fprintf(stderr, "%s\n", answer.error_message);
		return MATHINATOR_EXIT_UNDEFINED;
	}
	printf("mathinator: %g\n", answer.result_val);
	return MATHINATOR_EXIT_SUCCESS;
}
	...
	// 						printf("%s ( %g ) = %g\n", subChoice == 1 ? "sin" : subChoice == 2 ? "cos"
	// 																		: subChoice == 3   ? "tan"
	// 																		: subChoice == 4   ? "cot"
	// 																		: subChoice == 5   ? "sec"
	// 																		: subChoice == 6   ? "cosec"
	...
	// 				for (int i = 0; i < n; i++) {
	// 					numscanned = scanf(" %lf", &arr[i]);
	// 				}
	// 				if (numscanned == 1) {
	// 					result = stats(arr, n, subChoice);
	// 					if (!isnan(result)) { // Checking if result is not 'NaN'
	// 						printf("%s of the given dataset = %g\n", subChoice == 1 ? "Mean" : subChoice == 2 ? "Median"
	// 																					   : subChoice == 3	  ? "Mode"
	// 																					   : subChoice == 4	  ? "Variance"
	// 																					   : subChoice == 5	  ? "Standard Deviation"
	// 																										  : "Invalid function",
	// 							   result);
	// 						// Using ternary operator to display function name for conciseness, even though switch case was better option for readability.
	// 					}
	...
	// 	if (!isnan(result)) {
	// 		displayHistory(result, history, &hCount);
	// 	}
	// 	printf("------------------------------------------------\n");
